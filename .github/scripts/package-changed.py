#!/usr/bin/env python3
"""Decide whether a push can possibly change the published distribution.
...
CI, examples/ or the spec copy cannot, and publishing anyway burns a PyPI
version number permanently on identical contents. A version on PyPI cannot be
reused even after a delete.

WHAT COUNTS is whatever reaches the sdist and wheel: barndoor/ (the generated
client and the hand-written barndoor/lib), plus the metadata and lockfile that
describe them. Everything else here — .github/, examples/, openapi.yaml, tests/
— is real content that belongs in the repository and is read by people, but a
consumer running `pip install` never sees it.
...
touching many files — and a regeneration touches every generated model. The keys
were absent rather than empty, so it reported "unchanged" about a push that
rewrote the whole client, and no prerelease has ever published from one. The
payload's file lists are best-effort with undocumented caps; `git diff` has
neither property.
